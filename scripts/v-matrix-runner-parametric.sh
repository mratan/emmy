#!/bin/bash
# Parametric V-protocol matrix runner.
#
# Usage: v-matrix-runner-parametric.sh <profile_path> <base_url> <run_label>
#   profile_path: absolute path to a profiles/<id>/<version> directory
#   base_url:     vLLM endpoint, e.g., http://127.0.0.1:8002 or :8005
#   run_label:    short tag for run dir naming, e.g., gemma-26b-moe-v2.1-postchat-templ
#                 V1 transcripts → runs/v1-matrix-<run_label>/
#                 V3 transcripts → runs/v3-matrix-<run_label>/
#
...
#   - timings.tsv
#
# Lift all timeouts. Per operator: don't stop for anything except a real crash.
#
# V-EXP v10 (2026-05-05): parameterized from the original Mistral-only runner.
# Used for re-validation of fixes (Gemma chat_template update + Mistral v2.1
# SP_OK removal + grep tool ctx-overflow fix).
...
PROFILE="${1:?profile path required (e.g., $REPO/profiles/mistral-medium-3.5/v2.1)}"
BASE_URL="${2:?base_url required (e.g., http://127.0.0.1:8002)}"
LABEL="${3:?run_label required (e.g., gemma-26b-moe-postchat-templ)}"

V1_RUNS_DIR="$REPO/runs/v1-matrix-$LABEL"
V3_RUNS_DIR="$REPO/runs/v3-matrix-$LABEL"
