...
- **Primary model (daily-driver):** `google/gemma-4-26B-A4B-it` v2 (MoE, 4B active, decode p50 ~36 tok/s thermal-validated). **Switched from Qwen 35B-A3B-FP8 v3.1 on 2026-04-28** based on the 4-profile V-protocol matrix in `.planning/phases/04.4-filesystem-memory-tool-append-only-prefix-compaction-polish-/runs/V-RESULTS-v8-matrix-complete.md`: Gemma MoE clears V1 memory adoption at 100% (20/20) vs Qwen MoE 55%; V3 rot protection 5/5 on both; Gemma's lower throughput (~36 vs ~48 tok/s) is the operator-accepted cost of the higher compliance. The Qwen-MoE 50-55% V1 ceiling is a Qwen-MoE-specific compliance gap, not architectural — see V-RESULTS-v8 §"What this changes vs v7" for the falsification of the active-params hypothesis.
- **Alternate MoE slot (opt-in via `/profile`):** `Qwen/Qwen3.6-35B-A3B-FP8` v3.1 (3B active, decode p50 ~48 tok/s thermal-validated). Faster but with the documented compliance gap; reach for it when throughput dominates and the workload is operator-monitored. Phase 4 SC-3 within-model role routing (`profiles/routes.yaml` plan/edit/critic variants) only activates when this profile is loaded — Gemma has no role variants yet (Phase 5 work).
- **Heavy-agent slot:** `Qwen/Qwen3-Coder-Next-80B-A3B-FP8` (RL-trained for tools, ~30–43 tok/s)
- **Phase 4.1 dense siblings (eval-only, opt-in via `/profile`, NOT daily-driver):** `Qwen/Qwen3.6-27B-FP8` (dense, decode p50 ~7.6 tok/s) + `google/gemma-4-31B-it` (dense, BF16→runtime FP8, decode p50 ~6.4 tok/s). Both bandwidth-bound by design on GB10 UMA — single-digit tok/s is **expected**, NOT a regression. Acceptance gate is thermal stability (preemptions=0, oom=0), NOT throughput. Each dense family ships two siblings: **`v1`** (KV-ceiling audit at gmu=0.86, ~110 GiB UMA reservation — preserved as bisection-result artifact) and **`v1.1`** (operational at gmu=0.55, the D-29-equivalent RAM-headroom retune; DEFAULT_VARIANT points here). `/profile qwen3.6-27b` and `/profile gemma-4-31b-it` resolve to v1.1 automatically. Phase 5 evaluates dense-vs-MoE on real coding tasks. Both denses cleared the V1 memory gate (Qwen 27B 100%, Gemma 31B 95%) per V-RESULTS-v8.
...
---
*Updated 2026-04-28 — daily-driver switched from Qwen 35B-A3B v3.1 to Gemma 4 26B-A4B v2 per V-RESULTS-v8 4-profile matrix.*
