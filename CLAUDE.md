...
**Done bar:** daily-driver replacement for Claude Code AND research-grade reproducible artifact. **No cloud INFERENCE in the critical loop** — the *primary* LLM is 100% local, air-gap tested. Two narrowly-scoped, default-OFF, observable egresses are authorized in the threat model:

1. **`web_search` via self-hosted SearxNG** — loopback-bound from the harness's POV (`127.0.0.1:8888`); SearxNG itself reaches search engines. Disable via `EMMY_WEB_SEARCH=off` or `bash scripts/stop_searxng.sh`. STRICT air-gap CI invisible to it (loopback). Phase 3.1 shipping default.
2. **`ask_claude` via Spark-side Claude Code OAuth (Phase 04.6)** — sidecar (`emmy_serve.swap.controller` on `127.0.0.1:8003`) spawns the locally-installed `claude --print` subprocess; that subprocess reaches `api.anthropic.com` over the operator's Pro/Max OAuth. **Distinct from `web_search` in shape:** this is the first authorized cloud-side INFERENCE bridge, NOT loopback-bound at the upstream — Anthropic genuinely sees the (scrubbed) prompt. **Default OFF at three independent layers:** sidecar `EMMY_ASK_CLAUDE` env unset → 503 (D-03); profile schema absent → tool not registered (D-04); profile `tools.ask_claude.enabled: false` even when block present → tool not registered (D-13). Gated additionally by D-10 privacy attestation (Pro/Max training-data toggle OFF), D-06 pre-send scrubber (reject-don't-redact, 11 default secret-shape classes), D-07 rate-limit (30/hour, 1 in-flight, 3s min-gap, 5/turn), D-09 argv+stdin invocation (never shell). Anthropic's 2026-02-20 ToS update sanctions subscription OAuth ONLY for the official CLI; subprocess invocation is functionally equivalent to the explicitly-supported "running the CLI remotely via SSH" path.

The "no cloud INFERENCE" thesis remains intact for the *primary* loop — every Emmy turn's chat completion is served by local vLLM. `ask_claude` is a deliberately escalation-shaped, sparsely-invoked sidecar bridge for hard reasoning beyond local capability, with full audit (sha256 fingerprints + lengths + timing in `~/.local/state/emmy/sidecar/ask_claude_events.jsonl`; raw content only when `EMMY_LOG_FULL=on`). See Pitfall #5 + `docs/runbook.md § ask-claude tool` + `.planning/phases/04.6-ask-claude-bridge/04.6-CONTEXT.md` for the full posture.
...
- **Cross-tier reasoning escalation (Phase 04.6):** `ask_claude` tool + `/ask-claude` slash command, sidecar-mediated via Spark-side Claude Code OAuth. **Default OFF at three layers** — sidecar `EMMY_ASK_CLAUDE=on` env required (D-03), profile `tools.ask_claude` block must exist (D-04), profile `enabled: true` must be flipped per bundle (D-13). Per Anthropic's Feb 2026 ToS update, subscription OAuth is sanctioned only for the official CLI — Emmy spawns it via subprocess (argv + stdin, never shell, D-09) which is functionally equivalent to the explicitly-supported "running the CLI remotely via SSH" path. Pre-send scrubber rejects 11 default secret-shape classes (reject-don't-redact, D-06), rate-limit 30/hour + 1 in-flight + 3s min-gap globally + 5/turn harness-side (D-07), audit JSONL with sha256 fingerprints + lengths + timing only (raw content gated by `EMMY_LOG_FULL=on`, D-08). Slash command bypasses model-side gating (operator-only, D-05); tool form opt-in per profile and harness-rate-limited. See `docs/runbook.md § ask-claude tool` + `.planning/phases/04.6-ask-claude-bridge/04.6-CONTEXT.md`.
...
5. **Hidden cloud dependencies** — `VLLM_NO_USAGE_STATS=1` required; HF gated models need auth even offline. Verify with explicit air-gap test. **Note (Phase 3.1):** SearxNG is NOT a hidden dep — it's a loopback egress we intentionally ship and document. What makes it acceptable: self-hosted, digest-pinned, disable-able (`EMMY_WEB_SEARCH=off` or stop the container), observable (all queries logged to `tool.web_search` events). Air-gap CI split into two validators: `ci_verify_phase3` (STRICT — zero outbound, for inference-posture gate) and `ci_verify_research_egress` (PERMISSIVE — SearxNG OK, blocks 12 cloud-inference endpoints). **Note (Phase 04.6):** `ask_claude` is the second authorized egress and the first that reaches a cloud-side inference API — distinct from SearxNG in that the upstream IS Anthropic, not loopback. What makes it acceptable: same disable-able + observable + default-OFF properties as SearxNG, plus three additional gates (privacy attestation D-10, pre-send scrubber D-06 reject-don't-redact, rate-limit D-07 30/hour). Air-gap CI invisibility is the same shape — STRICT validator gates outbound sockets from the *harness*; `claude` subprocess spawned from the sidecar is outside its perimeter. The ToS-policy compliance leg is novel: Anthropic's 2026-02-20 update sanctions subscription OAuth only for the official CLI, so emmy spawns it via subprocess (argv + stdin) rather than re-implementing the OAuth client (which would violate the ToS). See `docs/runbook.md § ask-claude tool` for the full default-trust-OFF-at-three-layers walk.
...
---
*Updated 2026-04-30 — Phase 04.6 ask-claude-bridge documented: second authorized egress (Spark-side Claude Code OAuth via sidecar) added to the air-gap thesis paragraph + Pinned Tech Stack + Pitfall #5; default OFF at three layers; primary loop remains 100% local.*
