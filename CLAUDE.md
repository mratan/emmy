...
- **Phase 4.1 dense siblings (eval-only, opt-in via `/profile`, NOT daily-driver):** `Qwen/Qwen3.6-27B-FP8` (dense, decode p50 ~7.6 tok/s) + `google/gemma-4-31B-it` (dense, BF16→runtime FP8, decode p50 ~6.4 tok/s). Both bandwidth-bound by design on GB10 UMA — single-digit tok/s is **expected**, NOT a regression. Acceptance gate is thermal stability (preemptions=0, oom=0), NOT throughput. Each dense family ships two siblings: **`v1`** (KV-ceiling audit at gmu=0.86, ~110 GiB UMA reservation — preserved as bisection-result artifact) and **`v1.1`** (operational at gmu=0.55, the D-29-equivalent RAM-headroom retune; DEFAULT_VARIANT points here). `/profile qwen3.6-27b` and `/profile gemma-4-31b-it` resolve to v1.1 automatically. Phase 5 evaluates dense-vs-MoE on real coding tasks; until then, daily-driver default stays on Qwen 35B-A3B v3.1.
