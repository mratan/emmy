...
- [ ] **PROFILE-06**: Profiles are immutable. **Behavioral changes** (modify/replace/remove an existing key, change a prompt's bytes, move a default) require a new version directory. **Strictly additive changes** (add a new top-level block whose absence already validates as `None`, with zero pre-existing bytes modified) recompute the bundle hash in place; eval-replay against the prior version-string remains semantically valid because absent → None handlers already exist (e.g. `04.4-03` added `context.memory:`, `04.5-02` added `subagents:`). When in doubt, cut a new version — paperwork is cheap; behavioral drift in observability/eval history is not.
