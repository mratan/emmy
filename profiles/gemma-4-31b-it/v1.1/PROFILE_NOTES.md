...
measured_values:
  gpu_memory_utilization: 0.55  # v1.1 RAM-headroom retune from v1's KV-ceiling 0.86 (D-29-equivalent)
  gpu_clock_p5_hour2_mhz: 2405  # inherited from v1 thermal pass (gmu=0.55 strictly less memory pressure than 0.86 → thermal cannot regress)
  gpu_clock_p50_hour2_mhz: 2496
  decode_throughput_p50_hour2_tokps: 6.3  # informational, inherited from v1 (NOT a gate)
...
# Gemma 4 31B-it — v1.1 Profile Notes

Phase 4.1 follow-up — RAM-headroom sibling of v1 per D-02 immutability. v1
stays byte-identical at the KV-ceiling `gpu_memory_utilization=0.86` (hash
`sha256:fe9eded6...`); v1.1 retunes that single field down to `0.55` so the
operational footprint targets ~70 GiB and leaves >40 GiB system headroom on
the DGX Spark 128 GiB UMA pool. Mirrors the Qwen 35B-A3B v3 → v3.1 D-29
move exactly: hand-edit of gmu acknowledged as a Pitfall #3 (RAM headroom)
override of Pitfall #1 (find_kv_budget.py sole-writer), with v1 preserved
as the bisection-result artifact.

## v1.1 RAM-headroom retune

| Field | v1 value | v1.1 value | Source / rationale |
|---|---:|---:|---|
| `engine.gpu_memory_utilization` | 0.86 | **0.55** | Same `0.55` Qwen 35B-A3B v3.1 chose post-D-29; live-observed RAM exhaustion (free -h showed ~1 GiB free, swap in use) when v1's gmu=0.86 was running; CLAUDE.md Pitfall #3 lesson reapplied to dense |

All other fields byte-identical to v1: `max_model_len: 131072`,
`kv_cache_dtype: fp8`, `enable_prefix_caching: true`, `enable_chunked_prefill:
true`, `max_num_batched_tokens: 8192`, `max_num_seqs: 4`, `load_format:
safetensors`, `quantization: fp8`, `tool_call_parser: gemma4`,
`reasoning_parser: gemma4`, `container_entrypoint_override: ""` (CRITICAL),
`quirks.strip_thinking_tags: true` (channel-bleed backstop), harness.yaml +
prompts/ + tool_schemas/ + grammars/ all unchanged.

### Why hand-edit doesn't violate Pitfall #1

`scripts/find_kv_budget.py` is the sole-writer for the *no-preempt KV ceiling*
— a measurement-driven value. v1 captures that measurement (gmu=0.86 with
full thermal evidence). v1.1 chooses a *different objective* (system RAM
headroom for operator comfort) — same exemption Phase 3.1 took for the
Qwen MoE daily-driver. The bisection result is preserved in v1's hash and
PROFILE_NOTES.md; v1.1 is honest about being a comfort-tune, not a
ceiling-tune.

### Inheritance vs re-validation

Thermal pass for v1 ran at gmu=0.86 with `preemptions_hour2=0` and
`oom_events=0`. At gmu=0.55, vLLM holds *less* memory, which can only
*reduce* memory pressure → thermal cannot regress. Same logic Phase 3.1
applied when shipping v3.1 without a fresh thermal — gmu=0.55 < gmu=0.86
→ inherited safely. If a future tweak goes the *other* direction (higher
gmu than v1), a fresh 2×2h thermal would be required.
