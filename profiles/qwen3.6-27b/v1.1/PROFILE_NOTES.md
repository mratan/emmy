...
measured_values:
  ...
  gpu_clock_p5_hour2_mhz: 2476  # inherited from v1 thermal pass (gmu=0.55 strictly less memory pressure than 0.86 → thermal cannot regress)
  gpu_clock_p50_hour2_mhz: 2476
  decode_throughput_p50_hour2_tokps: 7.6  # informational, inherited from v1 (NOT a gate)
...
# Qwen3.6-27B-FP8 — v1.1 Profile Notes
...
`sha256:c3ccf1e1...`); v1.1 retunes that single field down to `0.55` so the
...
true`, `max_num_batched_tokens: 8192`, `load_format: fastsafetensors`,
`quantization: fp8`, `tool_call_parser: qwen3_coder`, `attention_backend:
flashinfer`, harness.yaml + prompts/ + tool_schemas/ + grammars/ all
unchanged.
...
applied when shipping v3.1 without a fresh thermal — gmu=0.55 < gmu=0.88
