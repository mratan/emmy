...
This is the **default** role variant of v3.1 (sibling bundle under `profiles/qwen3.6-35b-a3b/v3.1-default/`). It is the **no-role-hint fallback** — applied when the harness's `before_provider_request` hook cannot classify the turn as `plan`, `edit`, or `critic`. Routes-mapped to `roles.critic` in `profiles/routes.yaml` as well (critic reviews benefit from the same balanced sampling as the general fallback).

**Sampling contract (harness-side overrides applied per-turn):**

| Field | Value | Source |
|-------|-------|--------|
| `sampling_defaults.temperature` | 0.2 | base v3.1 parity — Qwen team blog 2026-04-16 coding default ([Qwen3.6 release blog](https://qwen.ai/blog?id=qwen3.6)) |
| `sampling_defaults.top_p` | 0.95 | base v3.1 parity |
| `sampling_defaults.top_k` | 40 | base v3.1 parity |
| `chat_template_kwargs.enable_thinking` | `false` | no reasoning turns on the default path — only `reason` variant unlocks thinking |

**Per-tool sampling:** unchanged from v3.1 base — `edit/bash/read` at `0.0` (Phase 2 D-11 + Qwen team determinism discipline). The other tools inherit `sampling_defaults.temperature: 0.2`.

**Citation:** The temperature=0.2 default carries forward from the Qwen team blog 2026-04-16 ([https://qwen.ai/blog?id=qwen3.6](https://qwen.ai/blog?id=qwen3.6), retrieved 2026-04-18 — base v3.1 PROFILE_NOTES.md §Sampling). No divergence from base v3.1; the variant exists so that routes.yaml can point at a named-default rather than the raw `v3.1` bundle, and so the OTel `emmy.profile.variant` attribute carries a distinct value per turn classification (D-12).

## Engine byte-identity (D-10)

`serving.yaml` is **byte-identical** to the `v3.1`, `v3.1-reason`, and `v3.1-precise` siblings. The CI test `tests/unit/test_variant_engine_byte_identity.py` enforces this invariant. Consequence: switching between `v3.1-*` variants at turn boundaries does **NOT** restart the vLLM engine — only harness-side state (sampling, `chat_template_kwargs`, prompts) mutates per turn.
