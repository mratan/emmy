...
## Variant Rationale (Phase 4 HARNESS-08)

This is the **precise** role variant of v3.1 (sibling bundle under `profiles/qwen3.6-35b-a3b/v3.1-precise/`). It is applied when the harness's `before_provider_request` hook classifies a turn as `edit` — routes-mapped to `roles.edit` in `profiles/routes.yaml`. Classification fires via message regex `^(edit|write|modify|rename|refactor|fix)\s` or via `turn.nextTool ∈ {edit, write}` on iteration-2+ refinement.

**Sampling contract (harness-side overrides applied per-turn):**

| Field | Value | Source |
|-------|-------|--------|
| `sampling_defaults.temperature` | 0.0 | CLAUDE.md "Hash-anchored edits stay default" + TOOLS-03 (Phase 2 locked edit-deterministic). Greedy decoding eliminates sampling-induced edit drift — the edit either produces the correct Hashline or it doesn't; nondeterministic retry is a correctness regression, not a feature. |
| `sampling_defaults.top_p` | 1.0 | unrestricted (inert when temperature=0 — documented explicitly so the value is auditable) |
| `sampling_defaults.top_k` | 40 | base v3.1 parity (inert when temperature=0) |
| `chat_template_kwargs.enable_thinking` | `false` | precise variant disables reasoning — edit turns should not ruminate. Pairs with `tools.per_tool_sampling.edit.temperature: 0.0` for end-to-end determinism. |

**Per-tool sampling (ALL tools at 0.0):** Overrides the v3.1 base (which had `edit/bash/read` at 0.0 and let the remaining 6 tools inherit the 0.2 default). The precise variant extends determinism to every tool:
- `edit: 0.0`, `bash: 0.0`, `read: 0.0` (unchanged from base)
- `write: 0.0`, `grep: 0.0`, `find: 0.0`, `ls: 0.0` (added for precise variant)
- `web_fetch: 0.0`, `web_search: 0.0` (added for precise variant — keeps search queries reproducible)

**Citations (Phase 4 SC-5 discipline):**
- CLAUDE.md "Hash-anchored edits stay default" (Phase 2 TOOLS-03): [docs.anthropic.com/claude/claude-code](https://docs.anthropic.com/claude/claude-code) (retrieved 2026-04-23)
- Phase 2 D-11 + Qwen team determinism discipline: `.planning/phases/02-pi-harness-mvp-daily-driver-baseline/02-CONTEXT.md` D-11
- Aider edit-format temperature convention: community default 0.0 for edit-mode LLMs ([https://aider.chat/docs/config/adv-model-settings.html](https://aider.chat/docs/config/adv-model-settings.html))

## Engine byte-identity (D-10)

`serving.yaml` is **byte-identical** to the `v3.1`, `v3.1-default`, and `v3.1-reason` siblings. The CI test `tests/unit/test_variant_engine_byte_identity.py` enforces this invariant. Consequence: switching to `v3.1-precise` at a turn boundary does **NOT** restart the vLLM engine — only harness-side state (sampling temperature 0.0, `chat_template_kwargs.enable_thinking=false`, per_tool_sampling broadened to every tool) mutates per turn.
