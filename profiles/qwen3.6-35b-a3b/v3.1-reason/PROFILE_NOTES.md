...
This is the **reason** role variant of v3.1 (sibling bundle under `profiles/qwen3.6-35b-a3b/v3.1-reason/`). It is applied when the harness's `before_provider_request` hook classifies a turn as `plan` (via message regex `^(plan:|think about|architect|design|strategy|outline)`) — routes-mapped to `roles.plan` in `profiles/routes.yaml`.

**Sampling contract (harness-side overrides applied per-turn):**

| Field | Value | Source |
|-------|-------|--------|
| `sampling_defaults.temperature` | 0.6 | Qwen team reasoning-turn sampling guidance ([Qwen3 blog](https://qwenlm.github.io/blog/qwen3/), retrieved 2026-04-23). Reasoning turns benefit from slightly higher entropy so the model explores multiple solution sketches before committing — CoT traces at temperature 0.2 tend to be "safe but narrow". |
| `sampling_defaults.top_p` | 0.95 | base v3.1 parity |
| `sampling_defaults.top_k` | 40 | base v3.1 parity |
| `chat_template_kwargs.enable_thinking` | `true` | unlock `<think>…</think>` reasoning turns in Qwen3.6 chat template — the distinguishing variant of reason mode ([Qwen readthedocs](https://qwen.readthedocs.io/) reasoning-effort + enable_thinking, retrieved 2026-04-23) |
| `advanced_settings_whitelist` | includes `reasoning_effort` | already on base v3.1 — available per-call without engine restart ([Aider model-settings guide](https://aider.chat/docs/config/adv-model-settings.html)) |

**Per-tool sampling:** unchanged from v3.1 base — `edit/bash/read` at `0.0` (determinism for tool calls even during reasoning turns). Rationale: the model may THINK at temperature 0.6, but when it emits a tool call the arguments must be deterministic. Per-tool overrides punch through `sampling_defaults.temperature` for the tool-emission half of a turn.

**Citations (Phase 4 SC-5 discipline):**
- Qwen team reasoning-turn sampling: [https://qwenlm.github.io/blog/qwen3/](https://qwenlm.github.io/blog/qwen3/) (retrieved 2026-04-23)
- reasoning_effort + enable_thinking guidance: [https://qwen.readthedocs.io/](https://qwen.readthedocs.io/) (retrieved 2026-04-23)

## Engine byte-identity (D-10)

`serving.yaml` is **byte-identical** to the `v3.1`, `v3.1-default`, and `v3.1-precise` siblings. The CI test `tests/unit/test_variant_engine_byte_identity.py` enforces this invariant. Consequence: switching to `v3.1-reason` at a turn boundary does **NOT** restart the vLLM engine — only harness-side state (sampling temperature 0.6, `chat_template_kwargs.enable_thinking=true`) mutates per turn.
